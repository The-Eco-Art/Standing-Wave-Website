...
class CarouselHub {
  ...
  register(slideNode) {
    ...
    console.log(`[CarouselHub] Registered: ${node.id} (L${node.level})`);
  }
  ...
  init() {
    ...
    if (!this.track) {
      console.error("[CarouselHub] Track not found");
      return;
    }
    ...
    console.log("[CarouselHub] Initialized with", this.nodes.size, "nodes");
  }
  ...
  autoRegisterSlides() {
    ...
    slides.forEach((slide, index) => {
      ...
      const metadata = {
        ...
        resonance: (slide.dataset.resonance || "").split(",").filter(Boolean),
      };
  ...
  setPhase(index) {
    ...
    console.log(`[CarouselHub] Phase shift: ${prevPhase} → ${index}`);
  }
  ...
  updateNavigation() {
    ...
    if (this.arrows.prev) {
      this.arrows.prev.style.opacity = this.currentPhase === 0 ? "0.3" : "";
    }
    if (this.arrows.next) {
  ...
  activateSlide(slide) {
    ...
    console.log(`[CarouselHub] Activated: ${slide.dataset.name}`);
  }
