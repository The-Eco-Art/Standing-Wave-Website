...
function initWaveVisualizer() {
  ...
  if (!canvas) {
    console.warn("[Wave Visualizer] Canvas not found - will retry when Node slide loads");
    return false;
  }
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          displayState.targetPhase = parseInt(entry.target.getAttribute("data-phase"));
  ...
  console.log("[Wave Visualizer Story Engine] Initialized");
  return true;
}
...
function checkSlideVisibility() {
  ...
  if (match) {
    ...
    isWaveSlideVisible = offset >= -slideWidth * 0.5 && offset <= slideWidth * 0.5;
  }
}
...
function drawWave() {
  ...
  displayState.phase = lerp(displayState.phase, displayState.targetPhase, 0.05);
  ...
  if (displayState.phase > 0.1) {
    ...
    let anchorOpacity = Math.min(1, Math.max(0, (displayState.phase - 0.1) / 0.9));
    ...
    if (displayState.phase > 4.5 && displayState.phase < 6) {
      anchorOpacity = Math.min(1, Math.max(0, 1 - (displayState.phase - 4.5)));
  ...
  if (displayState.phase > 1.1) {
    // Calculate opacity
    const nodeOpacity = Math.min(1, Math.max(0, (displayState.phase - 1.1) / 0.9));
  ...
  if (displayState.phase > 2.1) {
    const waveOpacity = Math.min(1, Math.max(0, (displayState.phase - 2.1) / 0.9));
    ...
    for (let x = leftX; x <= rightX; x += 2) {
      ...
      let structuralWave = Math.sin(relativeX * Math.PI * 2) * Math.sin(timeScale * 2.5);

      // Calculate Chaotic Wave (Broken boundaries and broken node, noisy)
      const noise = (Math.sin(relativeX * 15 + timeScale * 12) + Math.cos(relativeX * 25 - timeScale * 18)) * 0.15;
      let chaoticWave = (Math.sin(relativeX * Math.PI) * Math.sin(timeScale * 4) + noise);
    ...
    if (displayState.phase >= 2.8 && displayState.phase <= 3.5 || displayState.phase > 6.5) {
      ctx.fillStyle = "rgba(255, 255, 255, 0.8)";
      ctx.shadowBlur = 20;
      ctx.shadowColor = "#ffffff";

      let peekOpacity = 1;
      if (displayState.phase < 3) peekOpacity = (displayState.phase - 2.8) * 5;
      if (displayState.phase > 3) peekOpacity = 1 - ((displayState.phase - 3) * 2);
      if (displayState.phase > 6.5) peekOpacity = 1;

      ctx.globalAlpha = Math.max(0, Math.min(1, peekOpacity)) * waveOpacity;

      // Left Intrinsic peak
      let leftPeakAmp = Math.sin(0.25 * Math.PI * 2) * Math.sin(timeScale * 2.5) * amplitude * rigidDamping;
      ctx.beginPath();
      ctx.arc(leftX + (waveWidth * 0.25), centerY - leftPeakAmp, 4 * uiScale, 0, Math.PI * 2);
      ctx.fill();

      // Right Extrinsic peak
      let rightPeakAmp = Math.sin(0.75 * Math.PI * 2) * Math.sin(timeScale * 2.5) * amplitude * rigidDamping;
      ctx.beginPath();
      ctx.arc(leftX + (waveWidth * 0.75), centerY - rightPeakAmp, 4 * uiScale, 0, Math.PI * 2);
      ctx.fill();
    }
